...
function ConfigurePermissions($directory,$user_id)
{
    ...
    try
    {
        ...
        $subject_group = "AD\FAIS Share USERFILESFS Modify" ; $subject_rights = "Modify"
        ...
        (Get-ACL -Path $directory).Access | Foreach-Object {
                                                       
                                                                if($_.IdentityReference -eq 'AD\FAIS Share USERFILESFS Modify')
                                                                {
                                                                    $acl.RemoveAccessRule($_)   
                                                                }
                                                                if($_.IdentityReference -eq 'BUILTIN\Users')
                                                                {
                                                                    $acl.RemoveAccessRule($_)
                                                                }
                                                                if($_.IdentityReference -eq 'AD\FAIS Share USERFILESFS Read Only')
                                                                {
                                                                    $acl.RemoveAccessRule($_)
                                                                }
        ...
        $subject_acl.SetAccessRuleProtection($true,$false)
